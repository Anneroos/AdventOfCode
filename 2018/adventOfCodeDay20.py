...
# def findAllPaths(listOfStrings,string):
#     outputList = np.array([])
#
#     print("given string",string)
#     index = string.find('(')
#     if index >= 0:
#         print('found ( at index', index, 'next index is ', index + 1 , string[index+1])
#         j = index +1
#         nrOfLefts = 1
#         while True:
#             if string[j] == ')':
...
#             if nrOfLefts <= 0:
#                 closingindex = j
#                 print('found closing ) at index', j)
#                 break
#             j += 1
#         betweenBrackets = string[index+1:closingindex]
#         for substring in findSplits(betweenBrackets):
#
#         restOfString = string[closingindex+1:]
#         print(string[index:closingindex+1])
#
#         return outputList, restOfString
#     else:
#         for beginstring in listOfStrings:
#             outputList = np.append(outputList, beginstring+string)
#         print('pure!')
#
#     return outputList

def findSplits ( string):
    # assume beginning and ending ( ) are gone
    ...
    return [string[0:slicePos], string[slicePos+1:]]

def splitRegExs(listOfStrings):
    outputList = np.array([])
    ready = True
    for string in listOfStrings:
        index = string.find('(')
        if index >= 0: # means there is a substring with (...)
            ready = False
            # print('found ( at index', index, 'next index is ', index + 1 , string[index+1])
            j = index +1
            nrOfLefts = 1
...
#
#
# NNNE(WNW|ENE)ENENE (EKE|(EJEJE|ER))
#
#
# NNNE - rest
#
...
#
#
# wat als ik iets me tnodes doe
# dan moet je daarna nog alle paden bij langs
