...
class IntCodeComputer:
    ...
    def readInput(self,position):
        if len(self.input) > 0:
            print(self.name, "reading",self.input[0])
            self.intCode[position] = self.input.pop(0)
        else:
            self.tryingToGetInput = True
            self.intCode[position] = -1

    def output(self, value):

        self.outputValue.append(value)
        if len(self.outputValue) == 3:
            packetQueue.append(self.outputValue)
            print("Sending package", self.outputValue)
            self.outputValue = []
...
Computers = []
for i in range(50):
    Computers.append(IntCodeComputer(input23.copy(),[i],"Computer " + str(i)))
NAT = [0,0,0]
...
while attempts < 10000000:
    ...
    for computer in Computers:
        computer.doAStep()
        opcodesDict[computer.opcode] = opcodesDict.get(computer.opcode,0) + 1
        # print(computer.name, computer.tryingToGetInput)
        if not computer.tryingToGetInput:
            allIdle = False
    # while len(packetQueue) > 0:
    #     packet = packetQueue[:3]
    #     packetQueueQ = packetQueue[3:]
    # print(packetQueue)
    ...
    if len(packetQueue) > 0:
        ...
        for packet in packetQueue:
            ...
            if packet[0] == 255:
                NAT = packet
                print("Updated NAT to", NAT)

            else:

                Computers[packet[0]].addInputArray([packet[1], packet[2]])
        packetQueue = []
    else:
        # print("All Idle? :", allIdle)
        # print(opcodesDict)
        if allIdle:
            allIdleCounter += 1
            print("All Idle!",allIdleCounter," Current NAT",NAT)

            if len(NAT)>2:
                if NAT[2] == previousNATy:
                    for computer in Computers:
                        computer.stop()
                    break
                    print("Same y Again! *********************", NAT)
                else:
                    print("***************** USE NAT ********************************************", NAT)
                    Computers[0].addInputArray([NAT[1],NAT[2]])
                    previousNATy = NAT[2]
